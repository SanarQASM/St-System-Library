...
public class accountController implements Initializable {
	...
	private boolean usernameSi=false;
	private boolean passwordSi=false;
		...
		task.setOnSucceeded(_ -> {
			Platform.runLater(() -> {
				try {
					...
					image = new Image(Objects.requireNonNull(getClass().getResource("/image/mainImageBookSystem.png")).toString());
	...
	private void checkUsernameSi(){
		Platform.runLater(() -> {
			if (si_username.getText().isEmpty()) {
				si_errorUserName.setText("Empty!");
			} else if (si_username.getText().length() >= 35) {
	...
	private void checkPasswordSi(){
		Platform.runLater(() -> {
			...
			} else if (si_passwordPass.getText().length() >= 35) {
	...
	private void checkAnswer() {
			Platform.runLater(() -> {
				...
				} else if (su_answer.getText().length() >= 45) {
	...
	private void checkPassword() {
			Platform.runLater(() -> {
				...
				if (!(su_passwordPass.getText().isEmpty())) {
					...
					if (su_passwordPass.getText().length() >= 35) {
	...
	private void checkUsername() {
			Platform.runLater(() -> {
				if (!(su_username.getText().isEmpty())) {
					...
					if (su_username.getText().length() >= 35) {
